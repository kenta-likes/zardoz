...
let size   = List.length sorted  in
  if size mod 2 = 1 then Program.set_output [(List.nth sorted (size/2))]
  else
  let g1 = float_of_string (List.nth sorted (size/2)) in 
  let g2 = float_of_string (List.nth sorted (size/2 - 1)) in 
